...
class LeetCode():
	...
	def getProblemContent(self, pb_num):
		...
		reg = re.compile('<[^>]*>')
		self.pb_content = reg.sub('',response['data']['question']['content']).replace("&amp;","&").replace("&quot;", '"').replace("&apos;", "'").replace("&gt;",">").replace("&lt;", "<").replace("&nbsp;", "").replace("&le;", "<=").replace("&ge;", ">=").replace("&#39;", "'").replace("&ne;", "≠").replace("&infin;", "∞").replace("&rarr;", " -> ")
